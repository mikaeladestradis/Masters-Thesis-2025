...
params = two_dimensional_system_pars.type_3_one_spike
...
# set up to plot
v_vals = np.linspace(-99/kv, 70/kv, points)
...
def twodim(t, X, injected_current, b, eps):
    v, w = X
...
class NonDimensionalTwoDim(System):
    def move():
        # I want: a picture of a single spike maybe even animated...
        t_span = [0, 1000]
        t_eval = np.linspace(t_span[0], t_span[1], points)
        init = [equilibria[0], plot_winf(equilibria[0])]

        plt.plot(plot_phi(v_vals), v_vals, color='black')
        plt.scatter(init[1], init[0], color='red', s=10, zorder=10)
        plt.scatter(plot_phi(fold_points[0]), fold_points[0], color='green', s=10, zorder=10)
        plt.scatter(plot_phi(fold_points[1]), fold_points[1], color='green', s=10, zorder=10)
        plt.ylim(-0.8, 0.4)
        plt.xlim(0, 0.3)
        plt.show()

        initial_picture = solve_ivp(twodim, t_span=t_span, y0=init, method="RK45", t_eval=t_eval, args=(0, beta, 0.001))

    def run():        
        # start off by plotting the manifold
        eq_curve_og = dvdt.subs(w, winf)
        eq_curve_og = eq_curve_og.subs(sympy_params_exc)
        eq_curve_og = eq_curve_og.subs(beta, params.get("beta"))
        equil = util.find_equilibria(eq_curve_og, v, eq_guesses, 0)
        initial_conditions = [equil[0], plot_winf_type(equil[0], 0, params.get("beta"))]
        solution = compute_solution(initial_conditions, I_stim, params.get("beta"), 0.0015)
        ...
        def step_protocol(t):
            ...
            for t in t:
                if t <= 1000:
                    ...
                    stim.append(I_stim/100)
            return stim

        fig, axs = plt.subplots(2, 1, figsize=(8, 6), sharex=True)

        axs[0].plot(solution.t, solution.y[0], color='black')
        axs[0].set_ylabel("v")
        axs[0].set_ylim(-0.9, 0.5)
        axs[0].set_title("Time trace and injected current")

        axs[1].plot(solution.t, step_protocol(solution.t), color='black')
        axs[1].set_xlabel("T")
        ...
        plt.tight_layout()
        plt.show()
        # plt.savefig("combined_timetrace_class_1")
        plt.close()
        ...
        for value in v_vals:
            if (value <= fold_points[1]):
                lower_stable_branch.append(value)
            elif (value >= fold_points[0]):
                upper_stable_branch.append(value)
            else:
                unstable_branch.append(value)
        

        upper_stable_phi = plot_phi(np.array(upper_stable_branch))
        lower_stable_phi = plot_phi(np.array(lower_stable_branch))
        unstable_phi = plot_phi(np.array(unstable_branch))

        # plt.plot(upper_stable_phi, upper_stable_branch, color='blue', label=r"$S^{\pm}$")
        # plt.plot(lower_stable_phi, lower_stable_branch, color='blue')        
        # plt.plot(unstable_phi, unstable_branch, color='red', label=r"$U$")
        # plt.plot(plot_phi_type(v_vals, 0, params.get("beta")), v_vals, color='grey', ls='--')
        plt.plot(solution.y[1], solution.y[0], color='black', label="trajectory")
        plt.scatter(initial_conditions[1], initial_conditions[0], color='pink', zorder=10, label='initial condition')
        # plt.plot(g_vals, v_vals, color='green', label=r"$w$ nullcine", zorder=2)
        plt.plot(plot_phi_type(v_vals, I_stim, params.get("beta")), v_vals, color='grey', label=r"critical manifold $M$")
        plt.scatter(plot_phi(equilibria[0]), equilibria[0],  marker='o', color='red', zorder=11, label='equilibrium')
        # if len(equilibria) > 1: 
        #     plt.scatter(plot_phi(equilibria[1]), equilibria[1],  marker='o', zorder=10, color='red')
        # if len(equilibria) > 2:
        #     plt.scatter(plot_phi(equilibria[2]), equilibria[2],  marker='o', zorder=10, color='red')
        plt.scatter(plot_phi(fold_points[0]), fold_points[0], color='green', zorder=10, label=r"$F^+$")
        plt.scatter(plot_phi(fold_points[1]), fold_points[1], color='purple', zorder=10, label=r"$F^-$")
        plt.xlim(-0.2, 0.4)
        plt.ylim(-1, 0.7)
        plt.xlabel("w")
        plt.ylabel("v")
        # plt.title(r"Phase space of Class 3, with $I=0.025$ stimulus input")
        # plt.legend(loc='upper right')
        plt.title(r"Trajectory in phase space of Class 3, with stimulus $I=0.025$")
        ...
        # eq_40 = util.find_equilibria(type_2_eq_curve, v, eq_guesses, 40/20)
        ...
        # final_40 = shoot_up(initial_conditions, 40/(20), -13)
        # final_50 = shoot_up(initial_conditions, 50/(20), -13)
        # final_80 = shoot_up(initial_conditions, 80/(20), -13)

        # upper_40 = reduced_flow_upper([final_40, initial_conditions[1]], 40/(20), -13)
        ...
        # solution_40_high_eps = compute_solution(initial_conditions, 40/20, -13, params.get('epsilon'))
        ...
        # solution_40_low_eps = compute_solution(initial_conditions, 40/20, -13, params.get('epsilon')*0.1)
        ...
        # plot_time_traces([solution_40_high_eps, solution_40_low_eps], "Time trace for stimulus 40 and beta -13")
        ...
        # plt.scatter(lower_40[1][-1], lower_40[0][-1], color='blue', label='Equilibrium', zorder=10)
        ...
        # setup_nullcline_plot(r"Nullclines and GSPT flow for I=40$\mu A/cm^2$ and beta=-13", "Nullclines and GSPT flow for I=40 and beta=-13")
        ...
        # plt.plot(solution_80_low_eps.y[1], solution_80_low_eps.y[0], color='blue', label="Solution for epsilon 0.0015")
        # for eq in eq_80:
        #     plt.scatter(plot_winf_type(eq, 80/(20), 0), eq, color='blue', zorder=10, s=18)
        ...
        # plt.plot(solution_40_low_eps.y[1], solution_40_low_eps.y[0], color='blue', label="Solution for epsilon 0.0015")
        # for eq in eq_40:
        #     plt.scatter(plot_winf_type(eq, 40/(20), 0), eq, color='blue', zorder=10, s=18)
        ...
        # plt.plot(solution_20_low_eps.y[1], solution_20_low_eps.y[0], color='blue', label="Solution for epsilon 0.0015")
        # for eq in eq_20:
        #     plt.scatter(plot_winf_type(eq, 20/(20), 0), eq, color='blue', zorder=10, s=18)
